...
/* ============================================================
   THE PUFF — a second body driven by the very same poses, so
   every animation the hero has comes across for free.
   ============================================================ */
const KP = {
  body: C('#f6a8c0'), bodyL: C('#ffd6e4'), bodyD: C('#d97a9c'),
  foot: C('#d94a52'), footD: C('#96262f'), footL: C('#f07a80'),
  eye: C('#2b2b56'), eyeL: C('#ffffff'), eyeB: C('#5a6fd0'),
  blush: C('#f4788f'), mouth: C('#8e2a44'), tongue: C('#f0808f'),
  out: C('#4a2030')
};
function puffFrame(po, mode) {
  const g = new Pix(30, 30);
  const OX = HERO_AX;
  const bob = po.bob || 0, lean = po.lean || 0;
  const cx = OX + lean;
  const full = mode === 'full', inhale = mode === 'inhale';
  const cy = 14 + bob + (full ? -1 : 0);
  const rx = full ? 8.6 : (inhale ? 7.8 : 7.2);
  const ry = full ? 8.4 : (inhale ? 6.6 : 7.0);
  const feet = po.feet || [[OX - 3, H_FEET], [OX + 3, H_FEET]];
  const hands = po.hands || [[OX - 6, cy + 2], [OX + 6, cy + 2]];

  /* back foot, then the body, then the front foot over it */
  g.ell(feet[0][0], feet[0][1] - 1, 3.6, 2.4, KP.footD);
  /* stubby arms */
  g.ell(hands[0][0], hands[0][1], 2.6, 2.2, KP.bodyD);
  g.ell(hands[1][0], hands[1][1], 2.8, 2.4, KP.body);

  /* the puff itself */
  g.ell(cx, cy, rx, ry, KP.body);
  g.ell(cx - rx * 0.34, cy - ry * 0.38, rx * 0.5, ry * 0.36, KP.bodyL);
  g.ell(cx + rx * 0.3, cy + ry * 0.45, rx * 0.5, ry * 0.28, KP.bodyD);
  if (full) for (let k = 0; k < 3; k++) {
    /* strain lines when something is held in there */
    g.line(cx - rx - 1, cy - 3 + k * 3, cx - rx - 3, cy - 4 + k * 3, KP.bodyD);
  }

  /* face, set toward the way it is looking */
  const ex = cx + 0.8;
  const eh = inhale ? 2.2 : 3.0;
  for (const s of [-1, 1]) {
    const px2 = ex + s * 2.6;
    g.ell(px2, cy - 2.2, 1.5, eh, KP.eye);
    g.ell(px2, cy - 3.4, 1.1, 1.3, KP.eyeL);
    g.ell(px2, cy - 0.6, 0.9, 0.9, KP.eyeB);
  }
  g.ell(ex + 5.2, cy + 0.6, 2.0, 1.3, KP.blush);
  g.ell(ex - 4.6, cy + 0.6, 1.6, 1.1, KP.blush);
  /* mouth */
  if (inhale) {
    g.ell(ex + 1.6, cy + 3.2, 3.2, 3.0, KP.mouth);
    g.ell(ex + 1.6, cy + 4.2, 1.8, 1.4, KP.tongue);
  } else if (full) {
    g.ell(ex + 1.4, cy + 3.4, 2.0, 1.0, KP.mouth);
  } else {
    g.ell(ex + 1.4, cy + 3.2, 1.2, 0.9, KP.mouth);
  }
  /* the front foot */
  g.ell(feet[1][0], feet[1][1] - 1, 3.8, 2.5, KP.foot);
  g.ell(feet[1][0] - 0.8, feet[1][1] - 1.8, 2.2, 1.2, KP.footL);

  g.shade({ top: 0.15, bot: 0.18, right: 0.09, left: 0.05 });
  g.outline(KP.out, false);
  return g;
}
function puffTopFrame(dir, i, atkA) {
  const g = new Pix(26, 26);
  const cx = 13, cy = 13;
  const ang = [Math.PI / 2, Math.PI, -Math.PI / 2, 0][dir];
  const fx = Math.cos(ang), fy = Math.sin(ang);
  const px = -fy, py = fx;
  const attacking = atkA !== undefined && atkA !== null;
  const swing = attacking ? 0 : Math.sin(i / 4 * TAU) * 2.2;
  const lunge = attacking ? Math.cos(atkA * 0.8) * 2 : 0;
  const bx = cx + fx * lunge, by = cy + fy * lunge;
  /* feet poking out the front */
  g.ell(bx + fx * 6 + px * 2.6, by + fy * 6 + py * 2.6, 2.8, 2.0, KP.footD);
  g.ell(bx + fx * 6 - px * 2.6, by + fy * 6 - py * 2.6, 2.8, 2.0, KP.foot);
  /* arms */
  const sa = attacking ? ang + atkA : ang + Math.PI / 2;
  g.ell(bx + Math.cos(sa) * 6, by + Math.sin(sa) * 6, 2.4, 2.2, KP.bodyD);
  g.ell(bx - Math.cos(sa) * 6 + fx * swing * 0.3, by - Math.sin(sa) * 6 + fy * swing * 0.3, 2.4, 2.2, KP.body);
  /* the puff from above */
  g.disc(bx, by, 7.2, KP.body);
  g.disc(bx - fx * 2 - px * 1.6, by - fy * 2 - py * 1.6, 3.6, KP.bodyL);
  g.disc(bx + fx * 3.4, by + fy * 3.4, 3.4, KP.bodyD);
  /* a hint of the face at the leading edge */
  for (const s of [-1, 1]) g.ell(bx + fx * 5.4 + px * s * 1.8, by + fy * 5.4 + py * s * 1.8, 1.1, 1.1, KP.eye);
  g.shade({ top: 0.15, bot: 0.18 });
  g.outline(KP.out, false);
  return g;
}
function starSprite(i, n) {
  ...
  g.poly(inner, C('#fff4d6'));
  g.ell(cx - 1.4, cy - 1.4, 1.6, 1.6, C('#ffffff'));
  g.outline(C('#8a5f26'));
  return g;
}

Art.buildKirby = function () {
  if (Art.kirby) return;
  const H = {
    anchor: Art.hero.anchor,
    idle: frames(8, i => puffFrame(poseIdle(i))),
    walk: frames(8, i => puffFrame(poseWalk(i))),
    run: frames(8, i => puffFrame(poseRun(i))),
    jump: [puffFrame(poseJump()).canvas()],
    fall: [puffFrame(poseFall()).canvas()],
    land: [puffFrame(poseLand()).canvas()],
    dash: [puffFrame(poseDash(0)).canvas(), puffFrame(poseDash(1)).canvas()],
    atk: frames(6, i => puffFrame(poseAtk(i))),
    pierce: [puffFrame(posePierce(0)).canvas(), puffFrame(posePierce(1)).canvas()],
  ...
  for (let d = 0; d < 4; d++) {
    T.walk.push(frames(8, i => puffTopFrame(d, i)));
    T.idle.push([puffTopFrame(d, 0).canvas(), puffTopFrame(d, 4).canvas()]);
    T.atk.push(ATK_TOP.map(ang => puffTopFrame(d, 0, ang).canvas()));
  }
  Art.kirby = H; Art.kirbyTop = T;
  Art.item.star = frames(6, (i, n) => starSprite(i, n));
};

/* ============================================================
   THE GOLD AVATAR — a full palette swap, built only if asked for
   ============================================================ */
