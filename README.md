...
Open `index.html` in a browser. Click once for sound, press START, watch the opening
story, pick one of the three save files, then pick a realm on the map.

## The opening story

START runs a short animation before the file screen. Four ancient warriors leave the green
wood, cut a way through the bears that bar the road, and cross the peaks, the drowned
halls, the ash, the snow and the sand. They climb an outcrop of trees, look out, and find
an archipelago that no map holds. Then something comes up behind them, and the screen goes
black before you see it.

The whole thing runs for about forty seconds. **SKIP** in the top right corner ends it at
once, and so does `Esc`, `Enter` or `Space`.
...
the place the ancient warriors found. Iron chains cross the page from edge to edge and a
padlock hangs over the medallion. Click it and the chart says **COMING SOON**.
